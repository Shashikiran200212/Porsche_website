import {
  ...
  const Page = () => {
    return (
      <div className="h-screen w-full bg-white overflow-hidden flex flex-col items-center relative" style={{ userSelect: "none" }}>
        ...
        <div className="w-full flex-grow relative">
          <Carousel>
            <CarouselContent>
              {[
                ...
                <CarouselItem key={alt} className="relative flex flex-col items-center justify-center w-full">
                  <img
                    src={src}
                    alt={alt}
                  ...
                  <div className="mt-5">
                    <button className="px-4 py-2 text-xs sm:text-sm md:text-base lg:text-lg xl:text-xl uppercase font-semibold bg-transparent border-2 border-black transition-transform transform hover:scale-95 text-black duration-200 hover:bg-white hover:text-black">
                      <a href="https://credit-card-animation-gold.vercel.app/">Book Now </a> 
                    </button>
                  </div>
                </CarouselItem>
              ))}
          ...
          <div className="p-4  md:px-24 text-[10px] text-neutral-500">
            Any Manufacturer's Suggested Retail Price including Value Added Tax (MSRP incl. VAT) listed is for informational purposes only and excludes transportation fees, 
            taxes (except for those stipulated otherwise), license, title, optional or regionally required equipment. In particular, the MSRP incl. VAT is exclusive the additional 
            consumption tax as required by the Notice regarding the Additional Levy of Consumption Tax on Super-luxury Cars, issued by the Ministry of Finance and State Administration of 
            Taxation and effective from 1 December 2016. The MSRP may be adjusted due to the VAT rate change effective from 1 April 2019 as required by the Public Announcement Concerning 
            Relevant Policies on Continuing VAT Reform. Please consult with authorized dealers for specific pricing information.
          </div>
        </div>
      </div>
    );
